...
Each R version is configured to use [public Posit Package Manager](https://packagemanager.posit.co) and configured for CRAN and Bioconductor. Additionally all R packages needed for the RStudio IDE integration are preinstalled into a site-library. For CRAN a time-based snapshot is defined that points to the snapshot that is at most 60 days older than the corresponding R release date. Furthermore, a global `renv` cache points to `/home/renv` and the `renv`-`pak` integration is enabled. 

The Python versions are setup to include the `jupyter`/`jupyterlab` integration including the ability to publish to Posit Connect. Each Python version uses `pypi` from [public Posit Package Manager](https://packagemanager.posit.co).
