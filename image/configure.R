...
sink(paste0("/opt/R/",currver,"/lib/R/etc/Renviron.site"), append=TRUE)
  ...
  cat("RENV_PATHS_CACHE=/shared/renv\n")
sink()
...
cat('options(HTTPUserAgent = sprintf("R/%s R (%s)", getRversion(), paste(getRversion(), R.version["platform"], R.version["arch"], R.version["os"])))')
