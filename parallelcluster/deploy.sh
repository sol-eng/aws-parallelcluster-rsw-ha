...
AMI="ami-0659773cb016e83d2"
AMI="ami-0befdea6dbec51b66"
REGION="eu-west-1"
...
if [[ $CONFIG == "easybuild" ]]; then
    EASYBUILD_SUPPORT=true
fi
...
ADPWSecret=` cd ../pulumi && pulumi stack output "ad_password_arn" --show-secrets -s $CLUSTERNAME `
CERT="${KEY}.pem"
...
RSW_DB_PASS=`cd ../pulumi && pulumi stack output rsw_db_pass --show-secrets -s $CLUSTERNAME`
...
SECURE_COOKIE_KEY=`cd ../pulumi && pulumi stack output secure_cookie_key --show-secrets -s $CLUSTERNAME`
...
cat scripts/install-pwb-config.sh | \
	...
	sed "s#EASYBUILD_SUPPORT#${EASYBUILD_SUPPORT}#g" | \
        sed "s#SSL_SUPPORT#${SSL_SUPPORT}#g" | \
...
pcluster create-cluster --cluster-name="$CLUSTERNAME" --cluster-config=config/cluster-config-wb.yaml --rollback-on-failure false --suppress-validators ALL 
